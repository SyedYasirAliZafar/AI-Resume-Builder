...
const SkillsForm = ({data, onChange}) => {
  ...
  return (
        ...
        {data.length > 0 ? (
            <div className='flex flex-wrap gap-2'>
                {data.map((skill, index)=>(
                    <span key={index} className='flex items-center gap-1 px-3 py-1 mt-3 bg-blue-100 text-blue-800 rounded-full text-sm'>
                        {skill}
    ...
    <div className='bg-blue-50 p-3 rounded-lg mt-5'>
        <p className='text-sm text-blue-800'><strong>Tip:</strong>Add 8-12 relevant skills. Include both technical skills (prgramming languages, tools) and soft skills (leadership, communication).</p>
    </div>
